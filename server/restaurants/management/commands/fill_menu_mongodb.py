...
from restaurants.models import Menu, MenuCategory, MenuItem
...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> str | None:
        ...
        with open(json_l_path, "r") as fp:
            self.stdout.write(
                f"Loaded {json_l_path} file.... starting to build BULK QUERY"
            )
            for line in fp:
                            ...
                            if isinstance(menu_items, dict):
                                ...
                                for menu_name, data in menu_items.items():
                                    ...
                                    if isinstance(data, dict):
                                        # The data only contains "Veg" and "Non-veg", not
                                        # "Egg" that's why only including this.
                                        food_type = (
                                            "V"
                                            if data.get("veg_or_non_veg") == "Veg"
                                            else "NV"
                                        )
        ...
        if BULK_MENU_QUERIES:
            ...
            try:
                self.stdout.write(
                    f"Inserting {len(BULK_MENU_QUERIES)} menu documents into MongoDB..."
                )

                Menu.objects.insert(BULK_MENU_QUERIES, load_bulk=False)

                self.stdout.write(
                    self.style.SUCCESS("Successfully inserted all the menu data!!!")
                )
                ...
                end_time = time.time()
                self.stdout.write(
                    self.style.SUCCESS(f"EXECUTION TIME: {end_time - start_time}")
                )
